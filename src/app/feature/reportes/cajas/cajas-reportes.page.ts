...
import { CashRegisterReport } from '../../../core/models/report.model';
...
@Component({
  selector: 'cash-register-report-page',
  imports: [BreadcrumbsNg, DatePipe, DecimalPipe, ReportPeriodFilter],
  templateUrl: './cajas-reportes.page.html',
})
export default class CajasReportesPage {
  private readonly reportsService = inject(ReportsService);
  private readonly realtimeService = inject(RealtimeService);
  private readonly destroyRef = inject(DestroyRef);

  protected readonly period = signal<ReportPeriod>(currentReportPeriod());
  protected readonly report = signal<CashRegisterReport | null>(null);
    ...
    try {
      this.report.set(
        await firstValueFrom(
          this.reportsService.findCashRegisters({ from: period.from, to: period.to }),
        ),
      );
    } catch (error) {
